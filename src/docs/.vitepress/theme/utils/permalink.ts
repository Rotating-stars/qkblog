...
export const generateString = (length: number) => {
  const charset = '0123456789abcdefghijklmnopqrstuvwxyz'; // 可用的字符集
...
const rusult = ref<Object>()

export const usePostsFor = async (pathArray:Array<string> , baseDir:string, isflag:boolean=false) => {
    if(isflag) {
        await pathArray.forEach(i=>{
            const { rewrites } = usePosts({srcDir:i, baseDir:baseDir})
            rusult.value = rewrites;
        });
    }
    return rusult.value;
}
